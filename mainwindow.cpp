...
//funkcja pokazująca czas na liczniku
void MainWindow::showTime()
{
    int minuty = ui->licznik->value();
    if (suiczer == true)
    {
    minuty = minuty - 1;
    ui->licznik->display(minuty);
    }
    if (suiczer == true && minuty==0)
    {
        //nowa funkcja submitująca bez okienka weryfikacyjnego
        autosubmit();
    }
}
...
//funkcja która auto submituje egzamin po przekroczeniu czasu
void MainWindow::autosubmit()
{
    suiczer = false;
    ui->blad->hide();
    ui->ileczasu->hide();
...
//    ui->licznik->close();
    QProcess process;
    process.startDetached("/bin/bash", QStringList()<< "-c" << "./"+zadanko+".sh");
    pasek();
    QTimer *timer=new QTimer(this);
    connect(timer,SIGNAL(timeout()),this,SLOT(wyniker()));
    timer->start(3000);
